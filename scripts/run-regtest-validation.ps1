...
if ($mode -eq "suite") {
    $suiteModes = @(
        "all", "direct", "mempool", "stratum", "vardiff", "encoding", "shutdown",
        "safety", "synthetic-gbt", "large-mempool", "stress", "p2p-fast", "lifecycle"
    )
    ...
    Write-Host "ALL 13 REGTEST MODES PASSED"
    exit 0
}
