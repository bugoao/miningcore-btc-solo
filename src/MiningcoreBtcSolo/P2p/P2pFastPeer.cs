...
public sealed class P2pFastPeer
{
    ...
    {
        var pos = 0;
        if (!ReadVarInt(payload, ref pos, out var count))
            return;
        var job = _engine.ActiveMiningJob;
        if (!job.Ready)
            return;
        ...
        for (ulong i = 0; i < count; i++)
        {
            if (pos + 80 > payload.Length)
                return;
            var header = payload.AsSpan(pos, 80).ToArray();
            pos += 80;
            if (!ReadVarInt(payload, ref pos, out var txnCount) || txnCount != 0)
                return;

            var ann = ParseHeader(header);
            // Announced block height ≈ current job height (the block we were mining) + i
            var annHeight = job.Height + (uint)i;
            SoloLog.Info("p2p fast headers",
                ("peer", _cfg.Bitcoind.P2pFastPeer?.Trim() ?? "unknown"),
                ("hash", ann.BlockHashHex),
                ("height", annHeight),
                ("nbits", ann.BlockNbits.ToString("x8")));
    ...
    private async Task HandleHeaderBytesAsync(byte[] header80, CancellationToken ct)
    {
        ...
        var peerLabel = _cfg.Bitcoind.P2pFastPeer?.Trim() ?? "unknown";
        var annHeight = job.Height;
        SoloLog.Info("p2p fast cmpctblock",
            ("peer", peerLabel),
            ("hash", ann.BlockHashHex),
            ("height", annHeight),
            ("nbits", ann.BlockNbits.ToString("x8")));
    ...
    private static (string PrevhashHex, string BlockHashHex, uint BlockTime, uint BlockNbits) ParseHeader(byte[] header)
    {
        var prevLe = header.AsSpan(4, 32).ToArray();
        var prevBe = Hex.ReverseCopy(prevLe);
        var hashLe = BitcoinEncoding.DoubleSha256(header);
        var hashBe = Hex.ReverseCopy(hashLe);
        var time = BinaryPrimitives.ReadUInt32LittleEndian(header.AsSpan(68, 4));
        var nbits = BinaryPrimitives.ReadUInt32LittleEndian(header.AsSpan(72, 4));
        return (Hex.Encode(prevBe), Hex.Encode(hashBe), time, nbits);
    }
    ...
    private async Task<(string Command, byte[] Payload)> ReadMessageAsync(NetworkStream stream, CancellationToken ct)
    {
        ...
        else if (cmd is "block" or "cmpctblock")
        {
            ...
            payload = await ReadPrefixAndDiscardAsync(stream, (int)len, 80, ct);
        }
        else
        {
    ...
    private static async Task<byte[]> ReadPrefixAndDiscardAsync(
    ...
    private static bool ReadVarInt(byte[] data, ref int pos, out ulong value)
    {
        ...
        if (prefix == 0xfd)
        {
            ...
            value = BinaryPrimitives.ReadUInt16LittleEndian(data.AsSpan(pos, 2));
            pos += 2;
            return true;
        }
        if (prefix == 0xfe)
        {
            ...
            value = BinaryPrimitives.ReadUInt32LittleEndian(data.AsSpan(pos, 4));
            pos += 4;
            return true;
        }
        if (pos + 8 > data.Length) return false;
        value = BinaryPrimitives.ReadUInt64LittleEndian(data.AsSpan(pos, 8));
        pos += 8;
        return true;
    }
}
