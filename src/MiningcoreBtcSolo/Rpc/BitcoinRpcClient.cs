...
public sealed class BitcoinRpcClient : IDisposable
{
    ...
    public async Task<string?> SubmitBlockAsync(string blockHex, CancellationToken ct = default)
    {
        var id = Interlocked.Increment(ref _id);
        using var req = new HttpRequestMessage(HttpMethod.Post, _url);
        req.Headers.Authorization = _auth;
        req.Content = SubmitBlockRpcContent.Create(id, blockHex);
...
internal sealed class SubmitBlockRpcContent : HttpContent
{
    ...
    private SubmitBlockRpcContent(byte[] prefix, string blockHex)
    {
        _prefix = prefix;
        _blockHex = blockHex;
    ...
    public static SubmitBlockRpcContent Create(long id, string blockHex)
    {
        ...
        return new SubmitBlockRpcContent(prefix, blockHex);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var chunk = ArrayPool<byte>.Shared.Rent(Math.Min(StreamChunkSize, _blockHex.Length));
        try
        {
            for (var offset = 0; offset < _blockHex.Length; offset += chunk.Length)
            {
                ...
                for (var i = 0; i < count; i++)
                    chunk[i] = (byte)_blockHex[offset + i];
    ...
    {
        length = checked((long)_prefix.Length + _blockHex.Length + Suffix.Length);
        return true;
    }
}
